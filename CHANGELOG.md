...
- Never push a local-only manifest past the integration branch. Every Syncwheel push, whichever command builds it (`int push`, `publish`, `reconcile --push`, `stack push`, `stack land`, channel and journal publication, coordination repair), passes one check before anything is sent: for a local-only manifest, a commit whose tree carries `.syncwheel/manifest.json` may only update the integration branch, and not when that branch is `defaults.base_branch` on the canonical remote, or a Syncwheel coordination state or claim ref. Stack, pull-request, channel and landing branches are refused. The canonical remote is recognized by its normalized push URL, so a second remote name for the same URL is the same remote. A manifest the integration base already carries unchanged is a product file and passes. Git-tracked and journal repositories are unaffected, and `int push --dry-run` runs the same check.
- Warn, in `validate`, when a local-only manifest publishes its active-active coordination state on the canonical remote, naming the remote and the state branch.
- Control commits use `Syncwheel Control <control@syncwheel.invalid>`; published control tips change SHA once. Upgrade all clones together. The commit keeps the replay parent's commit time in `+0000`, pins `i18n.commitEncoding=UTF-8` and clears `GIT_CONFIG_PARAMETERS`, so neither a clone configured with another commit encoding nor an ambient one computes a different SHA. The formula is recorded as `control_identity` in the persistence intent; an intent written by an older version still recovers, and a control tip published by an older clone is still recognized.

## 0.43.35 - 2026-09-22

- Scope a coordinated `stack close` to the closed stack. The close publishes the published coordination state minus that stack, so an unpublished local change to another stack no longer blocks it and is not published by it, and a stack another clone published stays published even when the local manifest lacks it. The local manifest is still saved without the closed stack.
- Make an interrupted published close retryable. Before any delivery or ancestry proof, the retry looks for the close's token in the state history and in the closed ref's claim history; intents written before 0.43.35 are checked in the claim history. A close that already landed completes locally without publishing again, whatever changed locally or on the delivery branch since, and refuses only a different `--reason`, naming the original one. When this clone declared the same stack id again after that close, or its published entry matches the local one, the old intent is recorded as recovered and the command closes the stack as a new operation. A close that never landed is abandoned and the retry writes a new intent against the current state, unless the stack's published entry changed since the interrupted attempt: then it stops with `close_superseded` and the next run starts fresh. A published close that cannot read the coordination state now shows the underlying error, and only an unreachable remote gets the restore-access retry line.
- Name the stack and commit when an integration projection replay fails. When the same whole-stack check that `stack close --reason absorbed` uses proves the stack is in the base, the error says so and names `syncwheel stack close <id> --reason absorbed`; that check now also works without a local stack branch. Every other failure names the stack, the commit, and either the conflicting paths, the other stack that already changed them, or the first line of Git's own error, plus the desk rebuild command.
- Name the pending stacks and their owners in revision-provider refusals. `integration declared stack(s) are missing from integration` and `active-active handoff manifest is not aligned with fresh coordination state` now list each pending stack with its state and owner (last local actor, last published state), say that the owner must publish or reset it, and point to `syncwheel handoff`. The handoff refusal also names `syncwheel stack set <id> --published` for a commit-only change whose published commits are present locally. `syncwheel int rebuild` and `syncwheel int push` are named only when the local and published snapshots are equal and just the control-manifest digest differs. Long lists end with `and N more stack(s)`, and the whole refusal stays inside the protocol's error-length limit.
- Name the owner when a coordinated publication is blocked by another stack's unpublished local change. The refusal suggests `syncwheel stack set <id> --published` only for a commit-only change whose published commits are present locally; otherwise it says the owner must publish or reset it and points to `syncwheel handoff`.
- Add `syncwheel stack set <id> --published` to drop an unpublished local commit change to a published stack. It resets the stack's commits to the published entry, records the previous commits and the actor in the ledger, and prints the `stack set` command, with its `--repo`, `--manifest`, or `--personal` option, that restores them. It refuses for an unpublished stack and for a difference in anything other than commits. When published commits are missing locally it fetches the coordination remote once; if they are still missing, it refuses and names the owner.
- List the local proposal against the published state in `handoff` text and in `handoff --json` under `coordination.local_proposal`.
- Record the acting agent as `actor` on stack create, add, set, capture, promote, close-intent, and push ledger events. Set `SYNCWHEEL_LANE_OWNER` to name the agent; otherwise the actor is the local user, host, and parent process id.
...
- Preserve a witnessed, squash-delivered integration prefix when a later
  unmerged stack adds new product changes; continue rejecting unowned changes
  after that prefix.
